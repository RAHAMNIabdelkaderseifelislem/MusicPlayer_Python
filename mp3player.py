...
class FrameApp(Frame):
    ...
    def add_to_list(self):
        ...
        :return: None
        """
        directory = askopenfilenames(filetypes=(("Music files", "*.mp3;"),("All files", "*.*") ))
        # appends song directory on disk to playlist in memory    
        self.playlist.append("".join(directory))
        self.output.delete(0.0, END)

        for key, item in enumerate(self.playlist):
            # appends song to textbox
            song = EasyID3(item)
            song_data = "".join(directory)
            direct,name = os.path.split(song_data)
